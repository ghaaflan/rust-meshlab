/**
 * MeshLab Filter C API Implementation
 */
...
MeshResult mesh_load(MeshHandle* mesh, const char* filename) {
    ...
    try {
        clear_error();
        int mask = 0;
        int result = vcg::tri::io::Importer<CMeshO>::Open(mesh->mesh, filename, mask);

        if (result != 0) {
            set_error(std::string("Load failed: ") + vcg::tri::io::Importer<CMeshO>::ErrorMsg(result));
            return MESH_ERROR_IO_FAILED;
        }
...
MeshResult mesh_save(const MeshHandle* mesh, const char* filename) {
    ...
    try {
        ...
        // Need non-const reference for Save
        CMeshO& m = const_cast<CMeshO&>(mesh->mesh);
        int result = vcg::tri::io::Exporter<CMeshO>::Save(m, filename, mask);

        if (result != 0) {
            set_error(std::string("Save failed: ") + vcg::tri::io::Exporter<CMeshO>::ErrorMsg(result));
            return MESH_ERROR_IO_FAILED;
        }
